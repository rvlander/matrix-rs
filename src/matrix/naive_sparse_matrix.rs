    ...
    where T: Zero + PartialEq + Copy
{
    ...
    fn element_wise_binary_op<F: Fn((&T, &T)) -> T>(self,
        ...
        let mut tmp: HashMap<(usize, usize), T> = self.values
            .iter()
            .map(|a| {
                (*a.0,
                 match remaining.remove(a.0) {
                     Some(b) => f((a.1, &b)),
                     None => f((a.1, &zero)),
                 })
            })
    ...
    fn element_wise_unary_op<F: Fn(&T) -> T>(self, f: F) -> NaiveSparseMatrix<T> {
        ...
        let f0 = f(&zero);
        if f0.is_zero() {
            NaiveSparseMatrix::new(self.m,
                                   self.n,
                                   self.values.iter().map(|a| (*a.0, f(a.1))).collect())
        } else {
            let res = self.indices_iter()
                .map(|a| {
                    (a,
...
impl<T> NaiveSparseMatrix<T> {
    fn new(m: usize, n: usize, values: HashMap<(usize, usize), T>) -> NaiveSparseMatrix<T>
        where T: Zero
    {
    ...
    fn remove_zeros(&mut self)
        where T: Zero
    {
        self.values = replace(&mut self.values, HashMap::new())
            .into_iter()
            .filter(|a| !a.1.is_zero())
            .collect()
    }
}


impl<T: Add<T, Output = T> + Copy> Add for NaiveSparseMatrix<T>
    where T: Zero + PartialEq
{
    type Output = NaiveSparseMatrix<T>;

    fn add(self, rhs: NaiveSparseMatrix<T>) -> NaiveSparseMatrix<T>
        where T: Zero + PartialEq
    {
        self.__add(rhs)
    }
...
impl<T: Add<T, Output = T> + Copy> Add<T> for NaiveSparseMatrix<T>
    where T: Zero + PartialEq
{
    type Output = NaiveSparseMatrix<T>;
...
impl<T: Sub<T, Output = T> + Copy> Sub for NaiveSparseMatrix<T>
    where T: Zero + PartialEq
{
    type Output = NaiveSparseMatrix<T>;

    fn sub(self, rhs: NaiveSparseMatrix<T>) -> NaiveSparseMatrix<T>
        where T: Zero + PartialEq
    {
        self.__sub(rhs)
    }
...
impl<T: Sub<T, Output = T> + Copy> Sub<T> for NaiveSparseMatrix<T>
    where T: Zero + PartialEq
{
    type Output = NaiveSparseMatrix<T>;
...
impl<T: Mul<T, Output = T> + Copy> Mul<T> for NaiveSparseMatrix<T>
    where T: Zero + PartialEq
{
    type Output = NaiveSparseMatrix<T>;
...
impl<T: Div<T, Output = T> + Copy> Div<T> for NaiveSparseMatrix<T>
    where T: Zero + PartialEq
{
    type Output = NaiveSparseMatrix<T>;
...
impl<T: Neg<Output = T> + Copy> Neg for NaiveSparseMatrix<T>
    where T: Zero + PartialEq
{
    type Output = NaiveSparseMatrix<T>;
